                ...
                <Grid item xs={6} md={6} sx={{ display: 'flex', minHeight: '100%', paddingBottom: '30px' }} justifyContent={"center"} alignItems={'center'}>
                    <Grid container item justifyContent={"center"} sx={{ display: 'flex' }}>
                        <Grid item xs={9}>
                            ...
                            <InputDefault action={(e) => setEmail(e.target.value)} value={email} label={"Insira seu email"} color={"secondary"} type={"email"} icon={<MailOutlineIcon style={{ width: '24x', height: '25px', padding: '0px 10px 10px 0px', color: '#757575' }} />} />
                            <LabelDefault label='Senha' />
                            <InputDefault action={(e) => setPassword(e.target.value)} value={password} color="secondary" label="Insira sua senha" type={showPassword ? 'text' : 'password'} icon={<span onClick={passwordVisibility}>{showPassword ? <RemoveRedEyeIcon style={{ padding: '0px 10px 10px 0px' }} /> : <VisibilityOffIcon style={{ padding: '0px 10px 10px 0px' }} />}</span>} />
